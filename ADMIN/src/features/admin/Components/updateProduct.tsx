...
const UpdateProduct = () => {
    ...
    const handleSelectChange = (value: string) => {
        setProductData({ ...productData, cat_id: value});
    };
    ...
    const handleProductCreate = async () => {
        try {
            ...
            const invoiceRef = ref(db, `product/${productId}`);
            await update(invoiceRef, data);
            enqueueSnackbar('Cập nhật thành công', { variant: 'success' });
            navigate('/admin/produc');
        } catch (error) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (productId) {
                ...
                get(child(dbRef, `product/${productId}`))
                    .then((snapshot) => {
                        if (snapshot.exists()) {
                            setSelectCategory(snapshot.val().cat_id);
                            setProductData({
                                price: snapshot.val().price,
                                prod_describe: snapshot.val().prod_describe,
                                prod_name: snapshot.val().prod_name,
                                // prod_quantity: snapshot.val().prod_quantity,
                                prod_id: snapshot.val().prod_id,
                                cat_id: snapshot.val().cat_id
                            });
                            setImages([snapshot.val().prod_image]);
